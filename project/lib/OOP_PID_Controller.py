...
Time Tracking (millis): Uses Python's time module to simulate Arduino's millis() function.
...
class PIDControl:
    def __init__(self, Kp=0.0, Ki=0.0, Kd=0.0, T=100, max_control=100, min_control=-100):
        """
        ...
        self.last_time = int(round(time.time() * 1000))  # initialize with current time in ms
    ...
    def compute(self):
        ...
        if delta_time >= self.T:
            ...
            # Limit the control signal to max and min bounds
            if self.control_signal > self.max_control:
                self.control_signal = self.max_control
            elif self.control_signal < self.min_control:
                self.control_signal = self.min_control
